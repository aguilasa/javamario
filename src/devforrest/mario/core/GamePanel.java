...
public class GamePanel extends JPanel implements Runnable {
	...
	private void gameUpdate() {
		...
		if (!gameOver) {
			...
			ScreenState ss = map.screenState;
			
			Iterator<Creature> itr = map.creatures().iterator();
			while(itr.hasNext()) {
				Creature c = itr.next();
				if(c.isAlive()) {
			...
			for(GameTile t : map.animatedTiles()) {
				t.update(period);
			}
	...
	private void gameRender() {
		...
		renderer.draw((Graphics2D) dbg, map, panelWidth, panelHeight);
	}
